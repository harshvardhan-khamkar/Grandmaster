...
export default function AIAnalysis() {
  return (
    <section id="analysis" className="relative py-32 bg-[#05070D] overflow-hidden">
      ...
      <div className="absolute top-1/2 left-0 w-[500px] h-[500px] bg-[#D4AF37] opacity-5 blur-[150px] -translate-y-1/2 rounded-full pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          >
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-none border border-[#D4AF37]/30 bg-[#D4AF37]/5 text-[#D4AF37] text-xs uppercase tracking-widest font-medium mb-6">
              <Zap className="w-3 h-3" /> Real-time Evaluation
            </div>
            <h2 className="text-4xl md:text-5xl font-heading font-bold text-white mb-6">
              See the board through the <span className="text-[#D4AF37]">eyes of an engine</span>
            </h2>
            ...
            <div className="space-y-6">
              {[
                ...
                <div key={i} className="flex gap-4">
                  <div className="flex-shrink-0 w-12 h-12 bg-white/5 border border-[#D4AF37]/20 flex items-center justify-center text-[#D4AF37]">
          ...
          >
            <div className="relative z-10 glass-card rounded-none p-2 transform transition-transform hover:scale-[1.02] duration-500 shadow-2xl shadow-[#D4AF37]/10">
              {/* Fake UI Header */}
              ...
              <div className="aspect-square bg-[#0A0D1A] rounded-xl mt-2 relative overflow-hidden flex flex-col">
                  ...
                  <div key={rank} className="flex-1 flex">
                    {Array.from({ length: 8 }).map((_, file) => (
                      <div key={file} className={`flex-1 ${ (rank + file) % 2 === 0 ? 'bg-[#1A1F35]' : 'bg-[#0F1322]' } relative`}>
                         ...
                         {rank === 4 && file === 4 && (
                           <div className="absolute inset-0 bg-[#D4AF37]/20 border-2 border-[#D4AF37] animate-pulse" />
                         )}
                      </div>
                    ))}
                  </div>
                ...
                <div className="absolute bottom-0 left-0 right-0 h-24 bg-gradient-to-t from-[#D4AF37]/20 to-transparent flex items-end">
